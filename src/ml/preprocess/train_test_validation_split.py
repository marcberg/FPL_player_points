...
def split_data(train_size=0.6, perform_crossvalidation=True, random_state=None):
    ...
    If cv is True, no test set is created for cross-validation.
    ...
    if perform_crossvalidation:
        ...
        # Only split into train and validation sets
        X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=(1-train_size), random_state=random_state)
        ...
        return {
            ...
            'val': (X_val, y_val),
            'test': None  # No test-set when performing cross-validation
        }
